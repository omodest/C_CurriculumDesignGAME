...
void init(){
	...
	// 如果文件中除了密码外还有数据，就构建链表
	if(getFileSize(fp)>32){  
		// 创建临时文件，用来保存解密后的数据。 
		fpTemp = tmpfile();
		// 将文件指针移到偏移量为32的位置处
		fseek(fp, 32, SEEK_SET); 
		// 将数据读取到缓冲区buffer，然后解密 
		while( (readCount=fread(buffer, 1, pwdLen, fp)) > 0 ){ 
			for(i=0; i<readCount; i++){
				// 解密操作，将缓冲区中的每个字节与当前密码字节进行异或操作 
				buffer[i] ^= pwdCurrent[i];  
			}
			// 将经过解密操作后的数据从缓冲区 buffer 写入到文件 fpTemp 中
			fwrite(buffer, 1, readCount, fpTemp);
		}
		// 将文件 fpTemp 中待写入的数据立即写入到磁盘，以确保加密后的数据被完整地写入到文件中。
		fflush(fpTemp);
...
void setPwd(int flag){
	...
	}else if(flag==PWD_ALTER && !checkPwd(PWD_ALTER)){  //校验原有密码
		exit(EXIT_SUCCESS);
	}

	do{
		// getpwd 函数的作用是隐藏地接收用户在控制台上输入的密码 
		if(flag == PWD_SET){
			printf("请输入密码：");
		...
		if(strcmp(pwdNew1, pwdNew2) == 0){
			// 计算new1数组的哈希值，存到new5中 
			MD5((unsigned char*)pwdNew1, (unsigned char*)pwdNewMD5);  
			fseek(fp, 0, SEEK_SET); // 文件指针指向开头 
			fwrite(pwdNewMD5, 32, 1, fp); // 将密码的哈希值写入文件 
			if(flag == PWD_SET){
				pause("提示：密码设置成功！按任意键进入软件...");
				strcpy(pwdCurrent, pwdNew1); // 修改pwdCurrent和 pwdLen
...
int checkPwd(int flag){
	...
	for(i=0; i<PWD_ERROR_TIMES; i++){
		...
		getpwd(pwd, PWD_LEN);
		MD5((unsigned char*)pwd, (unsigned char*)pwdMD5); // 获取刚刚输入的密码或旧密码 
